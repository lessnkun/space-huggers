...
class Character extends GameObject 
{
    ...
    update() 
    {
        ...
        if (this.climbingLadder)
        {
            ...
            this.velocity.x += .02*delta*abs(moveInput.y);
            moveInput.x *= .2;
        }
        else
        {
        ...
        // save old pos for walk cycle speed
        ...
        {
            if (this.weapon)
                this.weapon.triggerIsDown = this.holdingShoot;

            if (this.grenadeCount > 0 && this.pressingThrow && !this.wasPressingThrow && !this.grendeThrowTimer.active())
            {
                // throw greande
                --this.grenadeCount;
                const grenade = this.playerIndex == 1 ? new TNTGrenade(this.pos, this) : new Grenade(this.pos, this);
    ...
    render()
    {
        ...
        const headColor = this.hatColor ? this.hatColor.scale(this.burnColorPercent(),1) : color;
    ...
    collideWithTile(data, pos)
    {
        ...
        {
            //debugPoint(pos, '#0ff')
            //debugPoint(vec2(pos.x,this.pos.y - this.size.y*.5), '#f0f')
            if (getTileCollisionData(pos.add(vec2(0,1))) == tileType_ladder)
                return 0;

            return !this.climbingLadder && pos.y < this.pos.y - this.size.y*.5;
        }
...
function alertEnemies(pos, playerPos, radius=2)
{
...
class Enemy extends Character
{
        ...
        else if (this.sawPlayerTimer.isSet() && this.sawPlayerTimer.get() < 10)
        {
            ...
            else if (timeSinceSawPlayer < 5)
            {
                ...
                if (!this.dodgeTimer.active())
                {
                    ...
                    // random jump
                    if (rand()<(this.type != type_elite ? .0005 : .005))
                    {
                        this.pressedJumpTimer.set(.1);
                        this.holdJumpTimer.set(rand(.2));
                    }
    ...
    {
        if (!this.isDead())
            this.alert(damagingObject ? damagingObject.pos : this.pos, 1);
...
class Player extends Character
{
    ...
    { 
        ...
        if (playerIndex == 1)
        {
            // Player 2: blue outfit
            this.color = new Color(.15, .3, .7); // blue clothes
            this.extraAdditiveColor = new Color(0, .05, .15, 0);
            this.hatColor = new Color(.2, .4, .9); // blue hat
        }
        ...
        this.holdingShield = 0;
        this.shieldBlockTimer = new Timer;
        this.persistent = this.wasHoldingJump = this.canBlink = 1;
        this.team = team_player;
        
        // Different weapons for different players
        if (playerIndex == 0)
            new RocketWeapon(this.pos, this); // Player 1: Rocket Tracking Weapon
        else if (playerIndex == 1)
            new SuperLaserWeapon(this.pos, this); // Player 2: Super Laser Weapon
        else
            new Weapon(this.pos, this); // Other players: Normal Weapon
    ...
    isShielding()
    {
        return this.playerIndex == 1 && this.holdingShield && !this.isDead();
    }
    ...
    blockProjectile(projectile)
    {
        ...
        const shieldDirection = this.getMirrorSign();
        const projectileDirection = projectile.velocity ? projectile.velocity.x : 0;
        if (projectileDirection * shieldDirection >= 0)
            return 0;

        const projectileOffsetX = projectile.pos.x - this.pos.x;
        if (projectileOffsetX * shieldDirection < -.05)
            return 0;

        this.shieldBlockTimer.set(.12);
        playSound(sound_walk, this.pos);
        this.applyForce(vec2(shieldDirection * .03, 0));
        return 1;
    }
    ...
    render()
    {
        ...
        const sizeScale = this.sizeScale;
        const shieldPos = this.pos.add(vec2(this.getMirrorSign(.38), .05).scale(sizeScale).rotate(-this.angle));
        const shieldAngle = this.angle + this.getMirrorSign(.12);
        const flash = this.shieldBlockTimer.active() ? .35 * (1 - this.shieldBlockTimer.getPercent()) : 0;
        drawRect(shieldPos, vec2(.24, 1.02).scale(sizeScale), new Color(.05, .45, .95, .85), shieldAngle);
        drawRect(shieldPos.add(vec2(this.getMirrorSign(.03), 0).rotate(-this.angle)), vec2(.14, .82).scale(sizeScale), new Color(.7 + flash, .95 + flash, 1, .75), shieldAngle);
        drawRect(shieldPos.add(vec2(this.getMirrorSign(.08), 0).rotate(-this.angle)), vec2(.06, .52).scale(sizeScale), new Color(.15, .8, 1, .9), shieldAngle);
    }
    ...
    update()
    {
            ...
            if (this.persistent && playerLives)
            {
                if (players.length == 1)
                {
                    if (this.deadTimer.get() > 2)
                    {
                        ...
                        playSound(sound_jump, checkpointPos);
                    }
                }
                else
                {
                    ...
                    if (minDeadTime > 2)
                    {
                        if (!hasLivingPlayers)
                        {
                            // respawn all
                            this.persistent = 0;
                            new Player(checkpointPos.subtract(vec2(this.playerIndex*.4,0)), this.playerIndex);
                            playSound(sound_jump, checkpointPos);
                        }
        ...
        // movement control
        // Player 1: WASD keys (left side), Player 2: Arrow keys (right side)
        if (this.playerIndex == 0)
        {
            // W=87, A=65, S=83, D=68
            this.moveInput.x = isUsingGamepad ? gamepadStick(0, 0).x : keyIsDown(68) - keyIsDown(65);
            this.moveInput.y = isUsingGamepad ? gamepadStick(0, 0).y : keyIsDown(87) - keyIsDown(83);
        }
        else if (this.playerIndex == 1)
        {
            // Arrow keys: Left=37, Right=39, Up=38, Down=40
            this.moveInput.x = gamepadStick(0, 1).x || keyIsDown(39) - keyIsDown(37);
            this.moveInput.y = gamepadStick(0, 1).y || keyIsDown(38) - keyIsDown(40);
        }
        else
        {
            this.moveInput.x = gamepadStick(0, this.playerIndex).x;
            this.moveInput.y = gamepadStick(0, this.playerIndex).y;
        }

        this.holdingShield = this.playerIndex == 1 && keyIsDown(32);
        if (this.holdingShield)
            this.moveInput.x *= .6;
        
        // jump
        // Player 1: W key (87), Player 2: Up arrow (38)
        this.holdingJump = (this.playerIndex == 0 && keyIsDown(87)) || 
                           (this.playerIndex == 1 && keyIsDown(38)) || 
        ...
        // controls
        // Player 1 (left side): Mouse左键/Q(81) 射击, 鼠标右键/E(69) 投掷, 鼠标中键/Shift(16) 闪避
        // Player 2 (right side): /(191) 射击, .(190) 投掷, ,(188) 闪避
        this.holdingShoot  = (this.playerIndex == 0 && (mouseIsDown(0) || keyIsDown(81))) || 
                             (this.playerIndex == 1 && keyIsDown(191)) ||
                             gamepadIsDown(2, this.playerIndex);
        this.pressingThrow = (this.playerIndex == 0 && (mouseIsDown(2) || keyIsDown(69))) || 
                             (this.playerIndex == 1 && keyIsDown(190)) ||
                             gamepadIsDown(1, this.playerIndex);
        this.pressedDodge  = (this.playerIndex == 0 && (mouseIsDown(1) || keyIsDown(16))) || 
                             (this.playerIndex == 1 && keyIsDown(188)) ||
        ...
        // jump when spawn in
        if (players.length > 1 && this.getAliveTime() < .1)
             this.pressedDodge = 1;
